...
export default {
  ...
  methods: {
    selectIndex(options) {
      ...
      let obj = this.customPlanDetail.periodList.find(
        item => item.planPeriod === 7
      );
      if (!obj) {
    ...
    convertPeriod(value, planPeriod, quantity) {
      if (value) {
        ...
        if (this.customPlanDetail.periodList) {
          ...
          if (obj) {
            if (obj.specificationList) {
 obj.specificationList = obj.specificationList.filter(
    ...
    createTableRow() {
      ...
      this.loading = true;
      createCustomPlan(this.customPlanDetail)
        .then(res => {
    ...
    getTableData() {
      getCustomPlansPages(this.searchRowData)
        .then(res => {
