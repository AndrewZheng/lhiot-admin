<template>
  <div class="m-role">
      ...
      <div class="modal-content">
        <Row>
          ...
          <i-col v-if="advertisementDetail.advertiseType === 'IMAGE'" span="21" class-name="mb10">
            <img :src="advertisementDetail.content" style="width: 300px;height: auto">
          </i-col>
    ...
    <Modal title="View Image" :mask-closable="false" :width="1200" v-model="relationTargetShow">
      <div class="modal-content">
        <Table border :columns="tempColumns" :data="tempModalTableData" @on-row-click="onRowClick"></Table>
      </div>
    </Modal>
  </div>
</template>
...
export default {
      ...
      relationType: [
        {
          ...
          columns: goods_on_sales_columns
        },
        {
        ...
        {
          value: 'STORE_LIVE_TELECAST',
          label: '门店直播',
          api: getProductShelvesPages
        },
        {
          value: 'CUSTOM_PLAN_SECTION',
          label: '定制版块',
          api: getProductShelvesPages
        },
        {
  ...
  computed: {
    advertisementPositionComputed() {
      let tempObj = this.advertisementList.find(
    ...
    relationTypeComputed() {
      let tempObj = this.relationType.find(
  ...
  methods: {
    ...
    searchAdvertisementRelation() {
      if (
        this.advertisementDetail.relationType === null ||
