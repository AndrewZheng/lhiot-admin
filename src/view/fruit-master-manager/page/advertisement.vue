<template>
  <div class="m-role">
      ...
      <div class="modal-content">
        <Row>
          ...
          <i-col v-if="advertisementDetail.advertiseType === 'IMAGE'" span="21" class-name="mb10">
            <img :src="advertisementDetail.content" style="width: 100px;height: auto">
          </i-col>
    ...
    <Modal :mask-closable="false" :width="1200" v-model="relationTargetShow" title="广告关联">
      <div class="modal-content">
        <Table :columns="tempColumns" :data="tempModalTableData" border @on-row-click="onRowClick"></Table>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      ruleInline: {
        ...
        content: [
          { required: true, message: '请填写广告文字内容或者上传图片' }
        ],
      ...
      relationType: [
        {
          ...
          label: '商品详情',
          api: getProductShelvesPages,
          columns: goodsOnSalesColumns
        },
        {
          value: relationType.PRODUCT_SECTION,
          label: '商品版块',
          api: getProductSectionsPages,
          columns: goodsModulsColumns
        },
        {
          value: relationType.CUSTOM_PLAN,
          label: '定制计划',
          api: getCustomPlansPages,
          columns: customPlanColumns
        },
        {
          value: relationType.CUSTOM_PLAN_SECTION,
          label: '定制版块',
          api: getCustomPlanSectionsPages,
          columns: customPlanSectionColumns
        },
        {
          value: relationType.ARTICLE_DETAILS,
          label: '文章详情',
          api: getArticlesPages,
          ...
          render: (h, params, vm) => {
            ...
            switch (row.advertiseStatus) {
              ...
              default:
                return (
                  <div>
                    <tag color='primary'>{row.advertiseStatus}</tag>
                  </div>
                );
            }
          }
  ...
  computed: {
    ...
    relationTypeComputed() {
      const tempObj = this.relationType.find(
        item => item.value === this.advertisementDetail.relationType
      );
      if (tempObj) {
    ...
    validityPeriodComputed() {
      if (
        !this.advertisementDetail.endAt &&
        !this.advertisementDetail.beginAt
      ) {
        return '永久有效';
      } else {
        return (
          this.advertisementDetail.beginAt + this.advertisementDetail.endAt
        );
      }
    }
  },
  ...
  methods: {
    ...
    onRowClick(row, index) {
      if (
        this.advertisementDetail.relationType ===
          relationType.PRODUCT_SECTION ||
        this.advertisementDetail.relationType ===
          relationType.CUSTOM_PLAN_SECTION
      ) {
        this.advertisementDetail.advertiseRelationText = row.sectionName;
      } else if (
        this.advertisementDetail.relationType === relationType.ARTICLE_DETAILS
      ) {
    ...
    handleEdit(params) {
      ...
      this.tempContent = null;
      this.loading = true;
      getAdvertisement({ id: params.row.id })
        .then(res => {
