  ...
  <div class="m-role">
    ...
    <Modal v-model="modalView" :width="1500" :mask-closable="false">
        ...
        <Row style="margin-top: 10px">
          <i-col span="10">
            <Row v-if="afterMsg.status === 'WAIT'">
              <i-col span="6"
                >退款总金额：{{ allMoney | fenToYuanDot2Filters }}</i-col
              >
            </Row>
            <Row v-else>
              <i-col span="6"
                >退款总金额：{{
                  afterMsg.refundAmount | fenToYuanDot2Filters
                }}</i-col
              >
            </Row>
          </i-col>
      ...
      <div v-show="afterMsg.status === 'WAIT'" slot="footer">
        <Button type="error" @click="handlePassAudit('refuse')"
          >拒绝售后</Button
        >
        <Button type="primary" @click="handlePassAudit('pass')"
          >审核通过</Button
        >
      </div>
    </Modal>
