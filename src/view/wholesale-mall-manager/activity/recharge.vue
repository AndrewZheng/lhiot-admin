            ...
            <Row>
              ...
              <i-col span="4">
                <FormItem :label-width="150" label="充值门槛（满xx元赠送）:" prop="rechargeAmount" >
                  <InputNumber :min="0" :value="rechargeAmountComputed" @on-change="rechargeAmountChange"></InputNumber>
                </FormItem>
              </i-col>
            </Row>
            <Row >
              <i-col class="ml15 brand-red">* 批量关联多张优惠券时，发放数量统一以此字段为准</i-col>
            </Row>
          </Form>
        </Row>
      </div>
...
const rewardColumns = [
  ...
  },
  {
    ...
    minWidth: 40,
    render(h, params, vm) {
      return <div>{params.row.rechargeAmount ? fenToYuanDot2(params.row.rechargeAmount) : 'N/A'}</div>;
    }
  },
  {
    title: '操作',
...
export default {
  ...
  computed: {
    rechargeAmountComputed(value) {
      return fenToYuanDot2Number(this.rewardConfig.rechargeAmount);
    }
  },
  created() {
    this.showBack = this.$route.name === 'wholesale-recharge';
  },
  mounted() {
  ...
  methods: {
    rechargeAmountChange(value) {
      this.rewardConfig.rechargeAmount = yuanToFenNumber(value);
    },
    getTableData() {
