  ...
  <div class="m-role">
    <Card>
      ...
      >
        ...
        <div slot="operations">
          <Button v-waves :loading="createLoading" type="success" class="mr5" @click="addShareReward">
            <Icon type="md-add"/>
            创建
          </Button>
          <Poptip
        ...
        <Row class-name="mb20">
          <i-col span="24">
            <Row>
              ...
              <i-col span="18"><img :src="shareRewardDetail.shareImageUrl" width="80%"></i-col>
            </Row>
          </i-col>
        </Row>
        <Row class-name="mb20">
        ...
        <Row class-name="mb20">
          <i-col span="24">
            <Row>
              ...
              <i-col span="18">{{ shareRewardDetail.rewardPositionStart }} -{{ shareRewardDetail.rewardPositionEnd }}</i-col>
            </Row>
          </i-col>
        </Row>
        <Row class-name="mb20">
...
export default {
        ...
        },
        {
          ...
          render: (h, params, vm) => {
            ...
            if (row.status === 'VALID') {
              ...
              return <div><tag color='primary'>{couponStatusConvert(row.status).label}</tag></div>;
            }
  ...
  methods: {
    ...
    createShareReward() {
      ...
      createShareReward(this.shareRewardDetail).then(res => {
    ...
    editShareReward() {
      this.modalViewLoading = true;
      editShareReward(this.shareRewardDetail).then(res => {
        this.modalEdit = false;
        this.modalViewLoading = false;
        this.getTableData();
      }).catch(() => {
    ...
    getTableData() {
      getShareRewardPages(this.searchRowData).then(res => {
