              ...
              <Row>
                ...
                <FormItem label="充值金额:" prop="rechargeFee">
                  <InputNumber
                    ...
                    placeholder="充值金额"
                    @on-change="rechargeFeeInputNumberOnChange"></InputNumber>
                </FormItem>
                  </Col>
                <Col span="8">
                <FormItem label="赠送金额:" prop="giveFee">
                  <InputNumber
                    ...
                    placeholder="赠送金额"
                    @on-change="giveFeeInputNumberOnChange"></InputNumber>
                </FormItem>
                  </Col>
                <Col span="8">
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (compareData(this.rechargeDetail.startTime, this.rechargeDetail.endTime)) {
    ...
    createRecharge() {
      ...
      createRecharge(this.rechargeDetail).then(res => {
    ...
    editRecharge() {
      this.modalViewLoading = true;
      editRecharge(this.rechargeDetail).then(res => {
        this.modalEdit = false;
        this.modalViewLoading = false;
        this.getTableData();
      }).catch(() => {
    ...
    getTableData() {
      getRechargePages(this.searchRowData).then(res => {
    ...
    getRelationTableData() {
      getRechargeRulePages(this.searchRelationRowData).then(res => {
        // 设置行是否可编辑
    ...
    modalHandleSave(params) {
      ...
      if (row.id == null || row.id == 0 || row.rechargeFee <= 0 || row.giveFee <= 0) {
    ...
    modalHandleDelete(params) {
      ...
      deleteRechargeRule({ ids: params.row.id }).then(res => {
    ...
    addTempData(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.addRelationDetail.giveFee > this.addRelationDetail.rechargeFee) {
            this.$Message.error('赠送金额不能大于充值金额');
            return;
          }
    ...
    createRelation() {
      this.modalViewLoading = true;
      createRechargeRule(this.addRelationDetail).then(res => {
        this.modalViewLoading = false;
        this.modalEdit = false;
        this.$Message.success('创建成功!');
        this.getRelationTableData();
