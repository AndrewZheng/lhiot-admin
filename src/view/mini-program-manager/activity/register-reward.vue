              ...
              <div style="margin: 10px;overflow: hidden">
                <Row type="flex" justify="end">
                  <Page
                    ...
                    @on-page-size-change="changeTemplatePageSize"></Page>
                </Row>
              </div>
              ...
              <Form ref="modalCreate" :model="addRelationDetail" :rules="relationRuleInline" :label-width="80">
                <Row>
                  ...
                  <FormItem label="单次发放数量:" prop="issuedNum">
                    <InputNumber :min="0" v-model="addRelationDetail.issuedNum" class="ml20" label="单次发放数量"></InputNumber>
                  </FormItem>
                   </Col>
                  <Col span="5">
                  <FormItem label="有效天数:" prop="effectiveDay">
                    <InputNumber :min="0" v-model="addRelationDetail.effectiveDay" class="ml20" label="有效天数"></InputNumber>
                  </FormItem>
                   </Col>
                  <Col span="5">
...
import { couponStatusConvert, couponTypeConvert, imageStatusConvert, couponScopeConvert } from '@/libs/converStatus';
...
export default {
  ...
  methods: {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          if (activityRegisterId === 0 || activityRegisterId === '') {
            ...
            return;
          } else if (couponTemplateIds.length === 0 || this.addRelationDetail.couponTemplateIds === '') {
    ...
    modalHandleSave(params) {
      ...
      editRegisterReward(row).then(res => {
        this.getRelationTableData();
      }).finally(res => {
    ...
    modalHandleDelete(params) {
      this.tempTableLoading = true;
      deleteRegisterReward({ ids: params.row.id }).then(res => {
    ...
    createRelation() {
      this.modalViewLoading = true;
      createRegisterReward(this.addRelationDetail).then(res => {
        this.modalViewLoading = false;
        this.modalEdit = false;
        this.$Message.success('创建成功!');
        this.getRelationTableData();
    ...
    getRelationTableData() {
      getRegisterRewardPages(this.searchRelationRowData).then(res => {
        // 设置行是否可编辑
