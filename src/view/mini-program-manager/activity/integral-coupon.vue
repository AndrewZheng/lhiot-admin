  ...
  <div class="m-role">
      ...
      >
        <div slot="searchCondition">
          <Row>
            ...
            <Select v-model="searchRowData.couponStatus" placeholder="优惠券状态" style="padding-right: 5px;width: 100px">
              <Option
   ...
   <Modal
    ...
    >
      ...
      <div class="modal-content">
        ...
        <Row class-name="mb20">
          <i-col span="12">
            <Row>
              ...
              <i-col span="16" v-if="addRelationDetail.couponType === 'DISCOUNT_COUPON'">{{ addRelationDetail.couponFee | fenToDiscountFilters }}</i-col>
        ...
        <Row class-name="mb20">
          ...
          <i-col span="12">
            <Row> 
              <i-col span="8">排序字段: </i-col>
              <i-col span="16">{{ addRelationDetail.rank }}</i-col>
            </Row>
          </i-col>
        </Row>

        <Row class-name="mb20" v-show="addRelationDetail.validDateType=='UN_FIXED_DATE' && addRelationDetail.source == 'SMALL'">
          <i-col span="12">
            <Row>
        ...
        <Row class-name="mb20" v-show="addRelationDetail.validDateType=='FIXED_DATE' && addRelationDetail.source == 'SMALL'">
          <i-col span="12">
            <Row>
        ...
        <Row class-name="mb20">
          <i-col span="12">
            <Row>
              <i-col span="8">兑换积分: </i-col>
              <i-col span="16">{{ addRelationDetail.points }}</i-col>
            </Row>
          </i-col>
          <i-col span="12">
            <Row> 
              <i-col span="8">促销积分: </i-col>
              <i-col span="16">{{ addRelationDetail.realPoints }}</i-col>
            </Row>
          </i-col>
        </Row>
        ...
        <Row class-name="mb20">
          <i-col span="12">
            <Row>
              <i-col span="8">每人限兑: </i-col>
              <i-col span="16">{{ addRelationDetail.receiveLimit }}</i-col>
            </Row>
          </i-col>
          <i-col span="12">
            <Row> 
        ...
        <Row class-name="mb20">
          <i-col span="12">
            <Row>
              <i-col span="8">券详情: </i-col>
              <i-col span="16">{{ addRelationDetail.couponDetail }}</i-col>
            </Row>
          </i-col>
          <i-col span="12">
            <Row> 
              <i-col span="8">温馨提示: </i-col>
              <i-col span="16">{{ addRelationDetail.couponReminderMsg }}</i-col>
            </Row>
          </i-col>
        </Row>
        <Row>
      ...
      <div class="modal-content">
        <Row>
          ...
          <div style="margin: 10px;overflow: hidden">
            ...
            <Row type="flex" justify="end" v-else>
              <Page
                ...
                @on-page-size-change="changeHdPageSize"></Page>
            </Row>
          </div>
          ...
          <Form ref="addForm" :model="addRelationDetail" :rules="relationRuleInline" :label-width="80">
            <Row>
              <i-col span="6" v-if="tempModalType == 'addTemplate'">
                <FormItem label="券有效期:" prop="useLimitType">
                  <Select v-model="addRelationDetail.validDateType" placeholder="券有效期类型" style="padding-right: 5px;width: 120px">
                      <Option
              ...
              <template v-if="addRelationDetail.validDateType=='UN_FIXED_DATE' && tempModalType=='addTemplate'">
                <i-col span="7">
                  <FormItem label="生效开始:" prop="beginDay">
                    <InputNumber :min="0" v-model="addRelationDetail.beginDay" label="生效开始" style="width: 160px"></InputNumber>
                  </FormItem>
                </i-col>
                <i-col span="7">
                  <FormItem label="有效天数:" prop="endDay">
                    <InputNumber :min="0" v-model="addRelationDetail.endDay" label="有效天数" style="width: 160px"></InputNumber>
                  </FormItem>
                </i-col>
              </template>
              <template v-if="addRelationDetail.validDateType=='FIXED_DATE' && tempModalType=='addTemplate'">
                <i-col span="7">
            ...
            <Row>
              <i-col span="6">
                <FormItem label="券使用范围:" prop="couponScope" :label-width="100" v-if="tempModalType=='addTemplate'">
                    {{ addRelationDetail.couponScope | couponScopeFilter }}
                </FormItem>
                <FormItem label="券使用范围:" prop="couponScope" :label-width="100" v-else>
                  <Select v-model="addRelationDetail.couponScope" placeholder="请选择" style="padding-right: 5px;width: 100px">
                    <Option
              ...
              <i-col span="6">
                <FormItem label="券使用限制:" prop="useLimitType" :label-width="100" v-if="tempModalType=='addTemplate'">
                    {{ addRelationDetail.useLimitType | couponUseLimitFilter }}
                </FormItem>
                <FormItem label="券使用限制:" prop="useLimitType" :label-width="100" v-else>
                  <Select v-model="addRelationDetail.useLimitType" placeholder="请选择" style="padding-right: 5px;width: 120px" disabled>
                    <Option
            ...
            <Row>
              <i-col span="12">
                <FormItem label="券详情：" prop="couponDetail">
                    <Input v-model="addRelationDetail.couponDetail" type="textarea" :autosize="{minRows: 3,maxRows: 8}" placeholder="请输入优惠券的详情"></Input>
                </FormItem>
              </i-col>
              <i-col span="12">
                <FormItem label="温馨提示：" prop="couponReminderMsg" :label-width="100">
                    <Input v-model="addRelationDetail.couponReminderMsg" type="textarea" :autosize="{minRows: 3,maxRows: 8}" placeholder="请输入优惠券的温馨提示"></Input>
                </FormItem>
              </i-col>
            </Row>
            <Row>
          ...
          *Tips：请先选择要关联的优惠券，然后输入关联配置信息，不可关联多个优惠券模板
        </Row>
      </div>
      <div slot="footer">
        <Button @click="handleAddClose">关闭</Button>
        <Button :loading="modalViewLoading" type="primary" @click="handleTemplateAdd">确定
        </Button>
      </div>
    </Modal>
    ...
    >
      ...
      <div class="modal-content">
        <Form ref="editForm" :model="addRelationDetail" :rules="relationRuleInline" :label-width="80">
          <Row>
            <i-col span="6">
          ...
          <Row>
              ...
              <template v-if="addRelationDetail.source == 'SMALL' && addRelationDetail.validDateType=='UN_FIXED_DATE'">
                <i-col span="7">
                  <FormItem label="生效开始:" prop="beginDay">
                    <InputNumber :min="0" v-model="addRelationDetail.beginDay" label="生效开始" style="width: 160px"></InputNumber>
                  </FormItem>
              ...
              <template v-if="addRelationDetail.source == 'SMALL' && addRelationDetail.validDateType=='FIXED_DATE'">
                <i-col span="7">
                  <FormItem label="生效时间:" prop="effectiveStartTime">
                    <DatePicker
                      :value="addRelationDetail.effectiveStartTime"
          ...
          <Row>
              <i-col span="6">
                <FormItem label="券使用范围:" prop="couponScope" :label-width="100" v-if="addRelationDetail.source == 'SMALL'">
                    {{ addRelationDetail.couponScope | couponScopeFilter }}
                </FormItem>
                <FormItem label="券使用范围:" prop="couponScope" :label-width="100" v-else>
                  <Select v-model="addRelationDetail.couponScope" placeholder="请选择" style="padding-right: 5px;width: 100px">
              ...
              <i-col span="6">
                <FormItem label="券使用限制:" prop="useLimitType" :label-width="100">
                  {{ addRelationDetail.useLimitType | couponUseLimitFilter }}
           ...
           <Row>
            <i-col span="12">
              <FormItem label="券详情：" prop="couponDetail">
                  <Input v-model="addRelationDetail.couponDetail" type="textarea" :autosize="{minRows: 3,maxRows: 8}" placeholder="请输入优惠券的详情，使用&符号换行"></Input>
              </FormItem>
            </i-col>
            <i-col span="12">
              <FormItem label="温馨提示：" prop="couponReminderMsg" :label-width="100">
                  <Input v-model="addRelationDetail.couponReminderMsg" type="textarea" :autosize="{minRows: 3,maxRows: 8}" placeholder="请输入优惠券的温馨提示，使用&符号换行"></Input>
              </FormItem>
            </i-col>
          </Row>
          <Row>
      ...
      <div slot="footer">
        ...
        <Button :loading="modalViewLoading" type="primary" @click="handleTemplateEdit">确定
        </Button>
      </div>
    </Modal>
...
import { couponStatusEnum, couponTypeEnum, couponScopeEnum, couponUseLimitEnum, validDateTypeEnum } from '@/libs/enumerate';
...
export default {
  ...
  computed: {
    ...
    systemCouponFixDate(){
      return this.tempModalType === 'addTemplate' && this.addRelationDetail.validDateType =='FIXED_DATE';
    },
    systemCouponUnFixDate(){
      return this.tempModalType === 'addTemplate' && this.addRelationDetail.validDateType =='UN_FIXED_DATE'
    }
  },
  mounted() {
  ...
  methods: {
    ...
    getTableData() {
      getCouponExchangePages(this.searchRowData).then(res => {
    ...
    replaceTextByTab(){
      ...
      if (this.addRelationDetail.couponDetail) {
        this.addRelationDetail.couponDetail = replaceByTab(this.addRelationDetail.couponDetail);
      }

      if (this.addRelationDetail.couponReminderMsg) {
        this.addRelationDetail.couponReminderMsg = replaceByTab(this.addRelationDetail.couponReminderMsg);
      }
    },
    replaceTextByTag(){
      // 活动规则换行用“&”拼接
      ...
      if(this.addRelationDetail.couponReminderMsg){
        this.addRelationDetail.couponReminderMsg = replaceByTag(this.addRelationDetail.couponReminderMsg);
      }

      if(this.addRelationDetail.couponDetail){
        this.addRelationDetail.couponDetail = replaceByTag(this.addRelationDetail.couponDetail);
      }
    },
    extraValidator(){
      if ( this.systemCouponFixDate && 
    ...
    createRelation() {
      ...
      createCouponExchange(this.addRelationDetail).then(res => {
    ...
    deleteTable(ids) {
      this.tempTableLoading = true;
      deleteCouponExchange({ ids }).then(res => {
